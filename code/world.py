...
class Level:
    def __init__(self, restart):
        ...
        # SoilLayer Instance attributes, passing in relevant Groups
        self._SoilLayer = SoilLayer(self._AllSprites, self._CollisionSprites)

        # Accessing the save file and pulling variables from it
        ...
        # AnimalList created to append animals to and modify their attributes after creation
        self._AnimalList = []
        ...
        self._Sky = Sky()
        self._Rain = Rain(self._AllSprites)

        # Rains if # is greater than 20
    ...
    # Toggle Inventory used to pause the game when Inventory is open
    def toggle_inventory(self):
        self._inventory_open = not self._inventory_open

    # Toggle Merchant used to pause the game when Merchant menu is open
    def toggle_merchant(self):
        self.shop_active = not self.shop_active

    # Plant Collision used for harvesting of plants
    ...
    def setup(self):
        if self._Level_init:
            self._Level_init = False  # Set to False so that this block is only called on initialization
        ...
        # Unload the house if the player enters the farm
    ...
    # Function to load the farm when the player enters
    def load_farm(self):
        self._Location = 'farm'
        self._Transition.play(self._Player)
        self.setup()

    # Function to load the house when the player enters
    def load_house(self):
    ...
    def run(self, DeltaTime):
        ...
        # Game continues if not paused
            ...
            # Open Inventory OR Merchant Menu OR draw all the sprites
            ...
            for animal in self._AnimalList:
                if animal._GoDir == "None":
                    ...
                    if animal._ImageFlip:  # Animal sprite flips to face left when they walk left
...
# Everything is drawn from Camera Group
